...
RUN if [ "$TARGETPLATFORM" = "linux/amd64" ]; then \
        ...
        sshpass -p "$user_pass" scp -o StrictHostKeyChecking=no ustore_deps_x86_linux.tar.gz runner@"$docker_ip":/home/runner/work/ustore-depst/ustore-depst; \
    elif [ "$TARGETPLATFORM" = "linux/arm64" ]; then \
        conan create ./ustore unum/arm_linux --build=missing && \
        cd ~/.conan && tar -czvf ustore_deps_arm_linux.tar.gz data/ && \
        sshpass -p "$user_pass" scp -o StrictHostKeyChecking=no ustore_deps_arm_linux.tar.gz runner@"$docker_ip":/home/runner/work/ustore-depst/ustore-depst; \
    fi